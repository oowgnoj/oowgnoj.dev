...
위의 코드에서 ?로 표시된 부분을 보자. 위에서 재귀함수를 정의할 때, 다음 번 실행할 함수에 인자를 넘겨줄 때는 ***축소된 인자***를 넘겨주는 경우가 많다고 했다. (`flip` 함수에서 `flip(list[:-1])`을 인자로 넘겨준 것 과 같은 원리)
